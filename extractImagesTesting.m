...
for subject = 251:318
	...
	for slice = (round(sz/2)-10) : (round(sz/2)+10)
	        ...
	        imwrite(uint16(sfieldmap*30),['../testingData/subject_' num2str(subject) '_fieldmap_slice_' num2str(slice) '.png'],'png','BitDepth',16);    
	   
	        sfMRI = fMRI(:,:,slice,1);
	        sfMRI = flipud(sfMRI');    
	        imwrite(uint16(sfMRI*50),['../testingData/subject_' num2str(subject) '_fMRI_slice_' num2str(slice) '.png'],'png','BitDepth',16);    
	    end
